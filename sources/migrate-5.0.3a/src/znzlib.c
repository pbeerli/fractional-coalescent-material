...
size_t znzread(void* buf, size_t size, size_t nmemb, znzFile file)
{
  ...
  if (file->zfptr!=NULL) {
    ...
    while( remain > 0 ) {
      n2read = (unsigned int) ((remain < ZNZ_MAX_BLOCK_SIZE) ? remain : ZNZ_MAX_BLOCK_SIZE);
       nread = gzread(file->zfptr, (void *)cbuf, n2read);
       if( nread < 0 ) return (size_t) nread; /* returns -1 on error */
...
size_t znzwrite(const void* buf, size_t size, size_t nmemb, znzFile file)
{
  ...
  if (file->zfptr!=NULL) {
    while( remain > 0 ) {
      n2write = (unsigned int) ((remain < ZNZ_MAX_BLOCK_SIZE) ? remain : ZNZ_MAX_BLOCK_SIZE);
       nwritten = gzwrite(file->zfptr, (void *)cbuf, n2write);

       /* gzread returns 0 on error, but in case that ever changes... */
       if( nwritten < 0 ) return (size_t) nwritten;
...
int znzprintf(znzFile stream, const char *format, ...)
{
  ...
  if (stream->zfptr!=NULL) {
    ...
    size = (int) strlen(format) + 1000000;  /* overkill I hope */
